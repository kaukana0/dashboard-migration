...
function beforeCurrentSelectionHappens(domElement, k,v) {
  ...
  if(ckeys.includes(k)) {
    domElement.selected = getIntersection(ckeys, domElement.selected)
  } else if(bkeys.includes(k)) {
    domElement.selected = getIntersection(bkeys, domElement.selected)
  } else {
    console.debug("bySelectDropDownbox logic: key doesnt exist:", k)
  }
...
export function getFrag(v) {
  ...
  for(let [key] of v.entries()) {
    retVal += "&"+"cbirth="+merged.get(key)
  }

  // TODO: where do we get dataset & dimension from?
